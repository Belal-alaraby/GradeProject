          ...
          {reasons.map((reason, index) => (
            <Reveal key={reason.id} delay={index * 150}>
              <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-2 group relative overflow-hidden flex flex-col h-full text-start">
                ...
                <div
                  className={`absolute -left-12 -top-12 w-40 h-40 rounded-full opacity-70 transition-transform duration-500 group-hover:scale-150 ${reason.shapeBg}`}
                ></div>

                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-6 shadow-md relative z-10 transition-transform duration-300 group-hover:scale-110 ${reason.iconBg}`}
                >
                  {reason.icon}
                </div>
                ...
                <p className="text-slate-500 leading-relaxed text-sm relative z-10 flex-grow">
                  {reason.desc}
                </p>
              </div>
            </Reveal>
