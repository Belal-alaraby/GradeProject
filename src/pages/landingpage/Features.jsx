...
export default function Features() {
  const services = [
    ...
    },
    {
      ...
      price: 250,
      icon: <Disc size={28} />,
      iconClass: "bg-slate-50 text-slate-600 border border-slate-200",
    },
    {
      id: 4,
      title: "غسيل السيارة",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Reveal key={service.id} delay={index * 150}>
              <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-blue-200 flex flex-col group">
                
                {/* icon */}
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-transform group-hover:scale-110 ${service.iconClass}`}
                >
                  {service.icon}
                </div>
                ...
                <div className="flex justify-between items-center pt-6 border-t border-slate-100">
                  ...
                  <a
                    href="#"
                    className="flex items-center gap-1 font-bold text-primary group-hover:text-secondary transition-colors text-sm"
                  >
                    احجز الآن
                    <ArrowLeft
                      size={16}
